...
def create_output(mesh, solver, output_folder, name):
    ...
    for i, n in enumerate(mesh.nodes):
        ...
        results.update({f"node {int(n[0])}": {"Coordinates": n[1:].tolist(),
...
def plot_geometry(mesh, output_folder, name):

    if not os.path.isdir(output_folder):
        os.makedirs(output_folder)

    fig, ax = plt.subplots()
    ax.plot(np.array(mesh.nodes)[:, 1], np.array(mesh.nodes)[:, 2], marker='o')
...
def make_movie(results, output_folder, name, fct=1000, tmp_folder="tmp", step=10):
    ...
    for i in range(0, len(time), step):
        ...
        x_t, y_t = [], []
        for node in results.keys():
            x_t.append(results[node]["Coordinates"][0] + results[node]["Displacement"][i, 0]*fct)
            y_t.append(results[node]["Coordinates"][1] + results[node]["Displacement"][i, 1]*fct)
